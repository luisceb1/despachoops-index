...
def test_safety_rejects_scan_root(tmp_path: Path):
    scan, _data, _reports, _latest, roots = _prod_like_tree(tmp_path)
    with pytest.raises(ReadOnlyViolation):
        assert_writable_output_path(scan / "doc.pdf", scan, roots)


def test_safety_allows_reports_and_latest(tmp_path: Path):
    scan, _data, reports, latest, roots = _prod_like_tree(tmp_path)
    assert_writable_output_path(
...
def test_safety_allows_data_dir(tmp_path: Path):
...
def test_safety_rejects_unconfigured_output(tmp_path: Path):
    scan, _data, _reports, _latest, roots = _prod_like_tree(tmp_path)
