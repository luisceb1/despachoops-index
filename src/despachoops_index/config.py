...
class AppConfig:
    ...
    def writable_output_roots(self) -> tuple[Path, ...]:
        return (self.data_dir, self.reports_dir, self.latest_dir)
    ...
    def latest_dashboard_path(self) -> Path:
        return self.latest_dir / "index_dashboard.xlsx"
...
def load_app_config(path: Path | str = "config.yaml") -> AppConfig:
    ...
    if raw.get("reports_dir"):
        reports_dir = Path(str(raw["reports_dir"])).expanduser()
    elif raw.get("shared_reports_dir"):
        reports_dir = Path(str(raw["shared_reports_dir"])).expanduser()
    elif shared_output_dir is not None:
        reports_dir = shared_output_dir / "reports"
    else:
        reports_dir = data_dir / "reports"

    if raw.get("latest_dir"):
        latest_dir = Path(str(raw["latest_dir"])).expanduser()
    elif raw.get("shared_latest_dir"):
        latest_dir = Path(str(raw["shared_latest_dir"])).expanduser()
    elif shared_output_dir is not None:
        latest_dir = shared_output_dir / "latest"
    else:
